...
class Controller extends \Framework\Controller
{
	...
	public function index(\Base $f3, $params)
	{
		...
		if (!empty($params['action'])) {
			...
			if (!$cache->exists('country.'.md5($params['action']), $result)) {
				$result['geoinfo'] = $this->geoinfo->findAll('country_name = ?', [$params['action']]);
				...
				$cache->set('country.'.md5($params['action']), $result, 3600);
			}
			...
			$context = [
				...
				'page' => [
					'title' => 'View IP',
					'body' => $this->view->render('app/module/country/view/view.php')
				]
			];
		} 
		// index
