...
class Controller extends \Framework\Controller
{
	...
	public function index(\Base $f3, $params)
	{
		...
		if (
			!empty($params['action']) &&
			$this->audit->ipv4($params['action']) &&
			$this->audit->ispublic($params['action'])
		) {

			$cache = \Cache::instance();

			if (!$cache->exists('ip.'.md5($params['action']), $result)) {

				$result = $this->domains->findAll('ip = ?', [$params['action']]);
				...
				$cache->set('ip.'.md5($params['action']), $result, 3600);
			}
			...
			if (!empty($params['sub_action']) && in_array($params['sub_action'], ['json', 'txt'])) {
				...
				foreach ($result as $key => $row) {
					...
					foreach ($result[$key] as $k => $v) {
						if (in_array($k, ['id', 'ipinfo_id', 'geoinfo_id', 'screenshot_id', 'weatherinfo_id', 'weatherinfo'])) {
							unset($result[$key][$k]);
						}
						// clean ipinfo
						if ($k == 'ipinfo') {
			...
			$context = [
				...
				'page' => [
					'title' => 'View IP',
					'body' => $this->view->render('app/module/ip/view/view.php')
				]
			];
		} 
		// index
			...
			$context = [
				...
				'page' => [
					...
					'body' => $this->view->render('app/module/ip/view/index.php')
				]
			];
		}
