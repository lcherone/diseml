...
class Controller extends \Framework\Controller
{
	...
	public function index(\Base $f3, $params)
	{
		...
		if (!empty($params['action'])) {
			...
			if (!$cache->exists('isp.'.md5($params['action']), $result)) {

				$result['ipinfo'] = $this->ipinfo->findAll('isp = ?', [$params['action']]);
				...
				$cache->set('isp.'.md5($params['action']), $result, 3600);
			}
			...
			$context = [
				...
				'page' => [
					'title' => 'View IP',
					'body' => $this->view->render('app/module/isp/view/view.php')
				]
			];
		} 
		// index
