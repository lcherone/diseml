	...
	<div class="card-content">
		<table class="table">
			...
			<tbody>
				...
				<tr>
					<td><?= $i ?></td>
					<td><?= (!empty($row['country_code']) ? '<i class="flag-icon flag-icon-'.htmlentities(strtolower($row['country_code'])).'"></i> ' : null) ?> <?= (!empty($row['country']) ? '<a href="/country/'.urlencode($row['country']).'">'.htmlentities($row['country']).'</a>' : null) ?></td>
